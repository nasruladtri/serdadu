...
# Script deployment untuk SERDADU
# Script ini menangani git pull dengan aman dan menjalankan semua langkah deployment
# Jalankan di server VPS sebagai root atau user dengan sudo

set -e  # Exit on error

echo "=== SERDADU Deployment Script ==="
echo ""
...
# 2. Backup perubahan lokal (jika ada)
echo ""
echo "2. Handling local changes..."
if ! git diff --quiet || ! git diff --cached --quiet; then
    echo "   ⚠️  Local changes detected. Stashing changes..."
...
# 7. Clear Laravel cache (PENTING: Clear route cache dulu!)
echo ""
...
echo ""
echo "📝 Notes:"
echo "   - Local changes were stashed (if any)"
echo "   - View stashed changes: git stash list"
echo "   - Apply stashed changes: git stash pop"
echo "   - Discard stashed changes: git stash drop"
echo ""
